...
medicaid_expansion %>%
  filter(year < 2014) %>%
  ggplot(aes(x = State, y = uninsured_rate, fill = uninsured_rate)) +
  geom_bar(stat="identity") +
  coord_flip() +
  theme_bw()
  ggtitle("Uninsured Rates before 2014")
```

The states with the highest uninsured rates prior to 2014 are Nevada, Florida and Texas. The lowest are Massachusetts, Hawaii and the District of Columbia. 
...
medicaid_expansion %>%
  filter(year < 2014) %>%
  ggplot(aes(x = State, y = population, fill = uninsured_rate)) +
  geom_bar(stat="identity") +
  coord_flip() +
  theme_bw()
  ggtitle("Number of Uninsured Americans before 2014")
...
medicaid_expansion %>%
  ...
  coord_flip() +
  theme_bw()
  ggtitle("Number of Uninsured Americans in 2021")
```

The states with the most uninsured Americans before 2014 are California, Texas, Florida and New York. Similarly, the states with the most uninsured Americans in 2020 are California, Texas, Florida and New York.
...
Initially I selected Maryland (adopted 01-01-2014) and Virginia (adopted 01-01-2019) as comparative units given their geographical proximity. The lines somewhat move together, however the pre-treatment dissimilarity between 2008-2010 are concerning. Given this, I decided to try out Utah and Colorado to ascertain if they have more aligned parallel trends.
...
- **Answer**: While straighforward to understand and implement, the parallel trends assumption is not a testable hypothesis. This means that we may 
...
**For this analysis**, we chose California, as it adapted the Medicaid expansion after January 1st, 2014. 

```{r}
# non-augmented synthetic control

# drop observations that did not have a Medicaid adoption date on record
medi2<- medicaid_expansion %>%
  drop_na(Date_Adopted)
...
**Answer**: The Average ATT estimate was -0.015, with an L2 imbalance of 0
...
**Answer**: In the augmented synthetic control, the Average ATT estimate was still -0.015, with an L2 imbalance of 0. 
